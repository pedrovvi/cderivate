...
void lexer_read_token(Lexer* lexer) {
  ...
  switch (current) {
    case '+': return lexer_push_token(lexer, PLUS_TOKEN);
    case '-': return lexer_push_token(lexer, MINUS_TOKEN);
    case '^': return lexer_push_token(lexer, EXPONENT_TOKEN);
    case 'x': return lexer_push_token(lexer, LETTER_TOKEN);
